...
if [ "$is_pr" = true ]; then
  ...
  git archive -o archive.tar.gz origin/"$src_branch" $(git diff --diff-filter=d --name-only origin/"$tg_branch" origin/"$src_branch") > /dev/null 2>&1
