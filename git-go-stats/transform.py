...
def get_complexity():
    ...
    cyclostats = subprocess.check_output(['./gocyclo', sys.argv[1]+'repo']).decode("utf-8") 
...
def get_duplicate_const_strings():
    ...
    cyclostats = subprocess.check_output(['./goconst', './'+sys.argv[1]+'repo/...']).decode("utf-8") 
...
def normalize_filename(filename):
    filename = filename.replace(sys.argv[1]+"repo", "")
    return filename
...
if __name__ == "__main__":  
    try:
        ...
        for f in files:
            result.append({
                'filename': normalize_filename(f),
                'cyclomatic_complexity': complexity[f] if f in complexity else 0,
                'duplicate_const_strings': duplicate_const_strings[f] if f in duplicate_const_strings else 0
            })
